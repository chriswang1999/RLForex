...
def Forex_reward_function(observation_history, action_history):
    ...
    # ##############
    # position = 2 - action_history[-1]
    # position = torch.tensor(position-1).to(device)
    # ##############
...
class DQNAgent(Agent):
    ...
    # def get_cum_reward(self, observation_history, action_history):
    #     reward_history = self.get_episode_reward(observation_history, action_history)
    #     self.eval_cummulative_reward += torch.sum(reward_history).to(device)
    #     return self.eval_cummulative_reward
...
#                next_feature_batch = torch.tensor(next_feature_batch, dtype=torch.float)
            ...
            # print(20 * "=")
            # print(self.model.layers[-1].weight)
            # print(self.model.layers[-1].weight.grad)
        ...
        if self.verbose and (self.num_episodes % self.print_every == 0):
            logging("dqn ep  " + str(self.num_episodes) + "    running loss  " + str(self.running_loss), self.log_path)
