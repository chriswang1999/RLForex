...
class ForexEnv(Environment):
    ...
    def __init__(self, cur = 'AUDUSD', lag = 32, min_history = 1000, mode = 'train', week = 1):
        self.ccy = cur
        self.lag = lag
    ...
    def get_features(self,_idx):
        ...
        if self.mode == 'eval':
            ...
            normalized = (feature_span-feature_span.mean())/feature_span.std()
        return 1e3*bid, 1e3*ask, 1e3*normalized
    ...
    def step(self, action):
        ...
        if self.mode == 'train':
            self.index = self.index % len(self.train)
            done = (self.index == len(self.train))
        elif self.mode == 'eval':
            self.index = self.index % len(self.eval)
