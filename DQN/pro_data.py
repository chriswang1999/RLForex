...
def gen_cols(Pad, cur, lag):
    ...
    for ccy in currency:
        if ccy == cur:
            ...
            _tmp = Pad[Pad['currency pair'] == ccy].sort_values(by=['timestamp'], ascending = True)
            for i in range(1,lag+1):
                colname3 = ccy + '_lag_' + str(i)
                mid =  pd.DataFrame(np.mean(np.asarray([_tmp['bid price'].values,_tmp['ask price'].values]), axis=0))
                tmp[colname3] = (np.log(mid) - np.log(mid.shift(i))).values
    ...
    try:
        ...
        tmp['time_4'] = np.sin(np.pi*0/60)*len(tmp['ss'])
...
if __name__=='__main__':
    ...
    # CreateFeature('EURUSD', 32, 1)
    # CreateFeature('USDJPY', 32, 1)
